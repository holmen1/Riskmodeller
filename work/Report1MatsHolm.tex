...
and since	   $\left(\Phi(Z_1),\Phi(Z_2)\right) \sim  U[0,1] \times U[0,1]$ .
...
\end{align}
a Gaussian copula $ \left(\tilde{S_1},\tilde{S_2}\right)$, where $(\hat{F}_X^\leftharpoonup$ and $\hat{F}_Y^\leftharpoonup$ are inverse of empirical cdf from $\hat{X}$ and $\hat{Y}$ respectively.
